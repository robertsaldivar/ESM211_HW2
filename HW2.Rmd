...
3.
The zero equalibrium is locally stable when P = ((1 + ahN)((1 + ahN) * (Kr) - (1 + ahN) * (2rN) - (1 + ahN) * (aKN)))/aK
...
fp1 <- function(P) ((1 + ahN)((1 + ahN) * (Kr) - (1 + ahN) * (2 * r * N) - (1 + ahN) * (aKN))) - aPK
...
fun_fp2 <- ggplot(data.frame(P = 0:7), aes(x = P)) +
  stat_function(fun = fp2, color = "blue") +
  ylab("N") + 
  geom_vline(xintercept =  3, color = "violet")
  

fun_fp2

fun_fp3 <- ggplot(data.frame(P = 0:7), aes(x = P)) +
  stat_function(fun = fp3, color = "green") +
  ylab("N") + 
  geom_vline(xintercept =   4, color = "violet")
